...
AstrBot：消息接入、权限、插件生态、群聊事件
        ↓
本插件注册的 openai_chat_completion provider
...
- 默认用 AstrBot 内置 `openai_chat_completion` 适配 Hermes 的 OpenAI-compatible API，更稳。
...
> **⏩ 老手可以直接跳到 [第 4 步](#第-4-步安装本插件)** — 只要你已经有运行中的 Hermes API Server。
...
9. 重启 AstrBot。
10. 用 `/hermes状态` 确认默认模型是否已经是 Hermes。
...
<details>
<summary>🔧 第 1~3 步：Hermes 部署指南（点击展开）</summary>
...
| `enable_provider` | `true` | 想让 AstrBot 使用 Hermes 时改成 true |
| `api_base` | `http://127.0.0.1:8642/v1` | Hermes API Server 地址。AstrBot 和 Hermes 不同机器时，把 IP 换成 Hermes 机器地址 |
| `api_key` | `sk-hermes-local` | 本机 Hermes 默认不校验 key，插件已内置占位值。若 Hermes 设置了真实 key，改成真实 key |
...
| `install_model_entry` | `true` | 同时创建 AstrBot 模型条目。如果不开，WebUI 模型列表可能看不到可选模型 |
| `set_as_default_provider` | `true` | 开启后 `/hermes安装提供商` 会直接把 AstrBot 默认聊天模型改成 Hermes |
| `disable_other_chat_models` | `false` | 切换默认时是否禁用其它聊天模型。一般不建议开 |
| `model_modalities_json` | `{"items":["text","tool_use"]}` | 写入 AstrBot 模型条目的能力列表。底层 Hermes 模型支持图片时可加入 `image` |
| `max_context_tokens` | `0` | 写入 AstrBot 模型条目的上下文长度。0 表示不限制 |
...
| 命令 | 作用 |
| --- | --- |
| `/hermes生态` | 显示插件用途和架构说明 |
| `/hermes配置` | 显示当前插件配置会生成的 provider JSON（隐藏真实 key） |
| `/hermes健康` | 检查 `/health` 和 `/v1/models`，确认 Hermes API 是否通 |
| `/hermes安装提供商` | 把插件配置写入 AstrBot 的 provider 和模型列表，并自动切默认模型 |
| `/hermes切换默认` | 单独把 AstrBot 默认聊天模型切到 Hermes |
| `/hermes状态` | 检查 provider source、模型条目、默认模型、api_base 是否正确 |

**新手推荐流程：**

```text
/hermes健康              ← 先确认 Hermes API 通
/hermes安装提供商         ← 一键写入 provider + 模型 + 切默认
```
...
```text
Loading model openai_chat_completion(hermes_agent/hermes-agent) ...
Selected openai_chat_completion(hermes_agent/hermes-agent) as default chat model provider
```

执行 `/hermes状态`，如果看到"是否正在默认使用 Hermes: 是"，就表示 QQ / WebChat 消息已经经过：

```text
AstrBot → openai_chat_completion → Hermes API Server → Hermes Agent
```

---

<details>
<summary>⚙️ 自定义 URL / Key / 底层模型切换（点击展开）</summary>
...
<details>
<summary>📋 手动 Provider 配置示例（点击展开）</summary>
...
请在插件配置里保持：`install_model_entry = true`，然后执行 `/hermes安装提供商`，再重启 AstrBot。
...
检查 Hermes 配置里的 `key` 和插件配置里的 `api_key` 是否一致。

### 5. `404 Not Found`

常见原因是 `api_base` 没写 `/v1`，或者反向代理路径写错。本插件会自动补 `/v1`，但如果你用了 Nginx / Caddy 反代，仍然要确认 `/v1/chat/completions` 被正确转发。

### 6. 调用很慢或超时

Hermes 是 Agent，不只是普通聊天补全。它可能会调用工具、读文件、联网、执行命令、生成计划。

建议 `timeout = 300 或 600`。如果你的 AstrBot 平台本身也有超时限制，也需要相应调大。
...
1. AstrBot 是否已经重启。
2. 是否真的选择了 `hermes_agent/hermes-agent` 这个模型。
3. `/hermes状态` 是否显示"是否正在默认使用 Hermes: 是"。
...
<details>
<summary>📦 AstrBot 插件市场 / 发布注意事项（点击展开）</summary>

本插件按 AstrBot 新版插件结构发布：

- 插件类继承 `Star`，不依赖已废弃的 `@register` 插件装饰器。
- 插件元数据以 `metadata.yaml` 为准。
- `metadata.yaml` 中的 `version` 使用普通版本号，例如 `0.2.7`，Release tag 可以使用 `v0.2.7`。
...
2. 如果 Hermes API Server 没设置 key，`api_key` 用默认 `sk-hermes-local`；如果设置了 key，就填 API key。
